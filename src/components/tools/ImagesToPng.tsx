  ...
  const convertToPng = async () => {
    try {
      const response = await runWithUploadProgress((onProgress, signal) =>
        imageService.convertToPng(selectedFiles, onProgress, signal)
      );
      ...
      {selectedFiles.length > 0 && (
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-yellow-600 text-white py-3 px-4 rounded-lg font-semibold hover:bg-yellow-700 disabled:bg-gray-300 transition-colors"
          >
